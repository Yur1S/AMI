...
begin
DataModule3.TPov.Sort := '[Дата поверки] ASC';
end;

procedure TFormPov.RadioButton2Click(Sender: TObject);
begin
DataModule3.TPov.Sort := '[Дата поверки] DESC';
end;

procedure TFormPov.DBGrid1CellClick(Column: TColumn);
begin
DataModule3.QSPov.Close;
DataModule3.QSPov.Parameters.ParamValues['N'] := DataModule3.TPov.FieldByName('Код прибора').Value;
DataModule3.QSPov.Open;
end;

procedure TFormPov.Button2Click(Sender: TObject);
begin
Edit1.Text:= IntToStr(DataModule3.QSPov.FieldByName('Срок поверки').Value);
end;
